...
my_net = torch.load(G_path, weights_only=False).to(device) 

d1_path = "./checkpoints/domain1.pth"
d1_net = torch.load(d1_path, weights_only=False).to(device)

d2_path = "./checkpoints/domain2.pth"
d2_net = torch.load(d2_path, weights_only=False).to(device)

d3_path = "./checkpoints/domain3.pth"
d3_net = torch.load(d3_path, weights_only=False).to(device)
...
for iteration_test, batch in enumerate(testing_data_loader,1):
    ...
    # Upscale the output using bicubic interpolation
    # Note: size is provided as (height, width), so (1080, 1920) corresponds to 1920x1080.
    out = F.interpolate(fake_b, size=(256, 256), mode='bicubic', align_corners=False)
